...
Did it on top already. Add a setup chunk that loads the tidyverse
packages.
...
![](week-4-visualize-exercises_files/figure-commonmark/add-label-3-1.png)
...
``` r
ggplot(mpg) +
  geom_histogram(binwidth = 2, aes(x=hwy))
```
...
ggplot(data = mpg) +
  geom_point(aes(x = displ, 
                           ...
                           color = class, 
                           alpha = 0.5)) +
  labs(x = "Engine Size", y = "Highway Mileage", color = "Class of Vehicle") +
  theme_classic()
```
...
ggplot(mpg) +
  ...
  guides(fill = "none") +
  labs(x = "Class of Vehicles", y = "Number of Vehicles in Sample") +
  theme_bw()
```
